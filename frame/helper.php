...
function view($template = '', $rewrite = false)
{
	return \frame\View::getInstance()->display($template, $rewrite);
}
...
function url($url = '', $param = [])
{
    if (strpos($url, 'http') === false) {
        $url = env('APP_DOMAIN').strtolower(\frame\Router::$_route['class'].DS).$url;
    }
    if (!empty($param))
