var INDEX = {
	...
	infoInit: function(data)
	{
		...
		var tmp = '';
		if (30 < data.server_load < 50) {
			tmp = 'style="color:#fc6d26;"'
		}
		if (data.server_load > 50) {
			tmp = 'style="color:red;"'
		}
        ...
        if (INDEX.net.table) {
        	INDEX.net.table.setOption({ xAxis: { data: INDEX.net.data.aData }, series: [{ name: '上行', data: INDEX.net.data.uData }, { name: '下行', data: INDEX.net.data.dData }] });
        }
	},
	start: function()
	{
	...
	net: {
        ...
        add: function(up, down) {
            ...
            if (_net.data.uData.length >= limit){
            	_net.data.uData.splice(0, 1);
            }
            if (_net.data.dData.length >= limit){
            	_net.data.dData.splice(0, 1);
            }
            if (_net.data.aData.length >= limit){
            	_net.data.aData.splice(0, 1);
            }
